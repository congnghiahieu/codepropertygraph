...
trait MethodEMT extends AnyRef with CfgNodeEMT with DeclarationEMT with HasAstParentFullNameEMT with HasAstParentTypeEMT with HasColumnNumberEndEMT with HasFilenameEMT with HasFullNameEMT with HasHashEMT with HasIsExternalEMT with HasLineNumberEndEMT with HasOffsetEMT with HasOffsetEndEMT with HasSignatureEMT
...
val Signature = "SIGNATURE" 
  ...
  object Properties {
    /** This field holds the FULL_NAME of the AST parent of an entity. */
val AstParentFullName = flatgraph.SinglePropertyKey[String](kind = 3, name = "AST_PARENT_FULL_NAME", default = "<empty>")
/** The type of the AST parent. Since this is only used in some parts of the graph,
the list does not include all possible parents by intention.
Possible parents: METHOD, TYPE_DECL, NAMESPACE_BLOCK. */
...
class Method(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 29.toShort , seq_4762) with MethodBase with CfgNode with Declaration with StaticType[MethodEMT] {
...
class NewMethod extends NewNode(29.toShort) with MethodBase with AstNodeNew with CfgNodeNew with DeclarationNew {
