...
trait LocationEMT extends AnyRef with HasClassNameEMT with HasClassShortNameEMT with HasFilenameEMT with HasLineNumberEMT with HasMethodFullNameEMT with HasMethodShortNameEMT with HasNodeLabelEMT with HasPackageNameEMT with HasSymbolEMT
...
class Location(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 26.toShort , seq_4762) with LocationBase with StaticType[LocationEMT] {
        ...
        nn match {
          case generated: NewLocation =>
            generated.node match {
              case Some(item) =>
                dstCast(offset) = item match {case newV:flatgraph.DNode => newV.storedRef.get; case oldV: flatgraph.GNode => oldV; case null => null}
                offset += 1
              case _ =>
            }
          case _ =>
...
class NewLocation extends NewNode(26.toShort) with LocationBase {
